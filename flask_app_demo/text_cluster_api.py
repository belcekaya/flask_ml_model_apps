...
def cluster():
    """
    Endpoint for clustering text data provided in a CSV file.

    Request Parameters:
    - dataset (file): The CSV file containing the text data.
    
    Query Parameters:
    - col (str): The column in the CSV file containing the text data (default: 'text').
    - no_of_clusters (int): The number of clusters for text clustering (default: 2).

    Returns:
    - File: A ZIP file containing the clustered data and associated reports.
    """
    ...
    for i in range(np.shape(kmeans.cluster_centers_)[0]):
        data_cluster = pd.concat([pd.Series(vectorizer.get_feature_names_out()),
    ...
    pd.DataFrame(clusters).to_excel(writer, sheet_name='Top_Keywords')
    ...
    memory_file.seek(0)
    response = make_response(send_file(memory_file, download_name='cluster_output.zip',
                                       as_attachment=True))
